...
then
	echo $red " "
	echo "L'adresse de diffusion ne peut être utilisée dans ce cas ..."
	echo $RESET " "
	exit 1
fi
}
...
#Sauvegarde aucien fichier de configuration en indiquant la date de l'enregistrement
cp /etc/dhcp/dhcpd.conf /etc/dhcp/dhcpd.conf.$(date +%d-%m-%Y:%H:%M:%S).bak
...
#Choix vider fichier de configuration
echo $okegreen " "
echo "Souhaitez-vous vider le fichier de configuration ? [O/N]"
echo $red
echo "[O] = Archive et date l'ancien fichier de configuration"
echo "[N] = Ajoute les adresses à la suite du fichier de configuration actuel"
echo $RESET " "
read vide
...
#Le fichier existe
	...
	if [ $vide = "N" ]
	then
		cat dhcpdbis.conf >> /etc/dhcp/dhcpd.conf
	fi
